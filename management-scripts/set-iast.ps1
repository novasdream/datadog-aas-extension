...
param (
    ...
    [Parameter(Mandatory = $true)][string]$DD_IAST_ENABLED 
)
...
Foreach ($webapp in @($allSites)) {
    ...
    if ($webapp.kind -eq "app") {
        ...
        Write-Host "Setting DD_IAST_ENABLED [${SiteName}]"
        az webapp config appsettings set -n ${SiteName} -g $ResourceGroup --settings DD_IAST_ENABLED=$DD_IAST_ENABLED | Out-Null
    }
}